...
arr_genero = movies.map {|k, v| v[:genre].capitalize}.uniq
puts arr_genero
...
arr_genero = movies.map {|k, v| v[:genre].capitalize} 
hash_genero = arr_genero.group_by {|x| x} 
hash_genero.map {|k, v| puts "Las películas de #{k} son: #{v.count}"}
...
fin = gets.chomp.to_i
puts "\n"
puts "Listado de películas entre #{ini} y #{fin}"
puts "\n"
